...
describe('AddCommand', () => {
  ...
  describe('execute', () => {
    it('should handle single file addition successfully', async () => {
      ...
      mockGitService.addFiles.mockResolvedValue(undefined);
    ...
    it('should handle multiple file addition successfully', async () => {
      ...
      mockGitService.addFilesAndCommit.mockResolvedValue('def456');
      
      const result = await addCommand.execute(['file1.txt', 'file2.txt', 'dir1'], { verbose: false });
      
      expect(result.success).toBe(true);
      expect(result.message).toContain('Successfully added 3 files');
      expect(mockConfigManager.addMultipleTrackedPaths).toHaveBeenCalledWith(['file1.txt', 'file2.txt', 'dir1']);
    });
    ...
    it('should handle invalid paths in batch operation', async () => {
      mockFileSystem.pathExists.mockImplementation((path: string) => {
        return Promise.resolve(path === '/test/workspace/valid.txt');
      });
    ...
    it('should handle already tracked paths', async () => {
      ...
      const result = await addCommand.execute(['already-tracked.txt', 'new-file.txt'], { verbose: false });
  ...
  describe('validateAndNormalizeMultiplePaths', () => {
    it('should validate multiple paths correctly', async () => {
      ...
      const result = await (addCommand as any).validateAndNormalizeMultiplePaths(['file1.txt', 'file2.txt']);
    ...
    it('should handle duplicate paths', async () => {
      ...
      const result = await (addCommand as any).validateAndNormalizeMultiplePaths(['file1.txt', 'file1.txt']);
    ...
    it('should detect already tracked paths', async () => {
      ...
      const result = await (addCommand as any).validateAndNormalizeMultiplePaths(['tracked-file.txt', 'new-file.txt']);
    ...
    it('should detect invalid paths', async () => {
      ...
      const result = await (addCommand as any).validateAndNormalizeMultiplePaths(['valid-file.txt', 'invalid-file.txt']);
  ...
  describe('executeMultipleAddOperation', () => {
    it('should execute atomic operation for multiple files', async () => {
      ...
      mockFileSystem.isDirectory.mockResolvedValue(false);
      mockSymlinkService.create.mockResolvedValue(undefined);
      mockGitService.addFilesAndCommit.mockResolvedValue('commit-hash');
      mockConfigManager.addMultipleTrackedPaths.mockResolvedValue({} as any);
      
      await (addCommand as any).executeMultipleAddOperation(['file1.txt', 'file2.txt'], { verbose: false });
      
      expect(mockFileSystem.moveFileAtomic).toHaveBeenCalledTimes(2);
      expect(mockSymlinkService.create).toHaveBeenCalledTimes(2);
      expect(mockGitService.addFilesAndCommit).toHaveBeenCalledWith(
        ['file1.txt', 'file2.txt'],
        'Add files to private tracking'
      );
      expect(mockConfigManager.addMultipleTrackedPaths).toHaveBeenCalledWith(['file1.txt', 'file2.txt']);
    });
    ...
    it('should rollback on failure', async () => {
      ...
      mockFileSystem.isDirectory.mockResolvedValue(false);
      mockSymlinkService.create.mockResolvedValue(undefined);
      mockGitService.addFilesAndCommit.mockRejectedValue(new Error('Git operation failed'));
      mockSymlinkService.remove.mockResolvedValue(undefined);
      
      await expect((addCommand as any).executeMultipleAddOperation(['file1.txt'], { verbose: false }))
        .rejects.toThrow('Git operation failed');
      
      // Verify rollback actions were called
      expect(mockSymlinkService.remove).toHaveBeenCalledWith('/test/workspace/file1.txt');
    });
  });
});