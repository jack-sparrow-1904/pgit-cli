...
describe('FileSystemService', () => {
  ...
  describe('writeFile', () => {
    ...
    it('should throw error when write fails', async () => {
      const error = new Error('Permission denied');
      (mockedFs.writeFile as jest.Mock).mockRejectedValue(error);
  ...
  describe('writeFileAtomic', () => {
    ...
    it('should cleanup temp file on error', async () => {
      ...
      expect(mockedFs.remove).toHaveBeenCalledWith(expect.stringContaining('/test/file.txt.tmp'));
    });
  });
  ...
  describe('ensureDirectoryExists', () => {
    ...
    it('should throw error when directory creation fails', async () => {
      const error = new Error('Permission denied');
      (mockedFs.ensureDir as jest.Mock).mockRejectedValue(error);

      await expect(fileSystemService.ensureDirectoryExists('/test/dir')).rejects.toThrow(
        'Permission denied',
      );
    });
  });
  ...
  describe('moveFileAtomic', () => {
    ...
    it('should rollback on failure', async () => {
      ...
      await expect(
        fileSystemService.moveFileAtomic('/source/file.txt', '/dest/file.txt'),
  ...
  describe('validatePathString', () => {
    ...
    it('should reject null or empty paths', () => {
      ...
      expect(() => fileSystemService.validatePathString(null as any)).toThrow(
    ...
    it('should reject paths with null bytes', () => {
      expect(() => fileSystemService.validatePathString('path\x00with\x00null')).toThrow(
  ...
  describe('error handling', () => {
    ...
    it('should handle EACCES errors gracefully', async () => {
      ...
      await expect(fileSystemService.writeFile('/protected/file.txt', 'content')).rejects.toThrow(
